//+------------------------------------------------------------------+
//| goldpulse_burst_v5.mq5                                           |
//| Edge first. Speed second.                                        |
//| M5/H1 trend bias + M1 candle/tick impulse + probe -> burst engine|
//| v5: per-bar throttle, RSI filter, scaled lots, partial close     |
//+------------------------------------------------------------------+
#property copyright "Wayne Heyliger / OpenAI"
#property version   "5.00"
#property strict
#property description "GoldPulse BurstTrend v5.00: per-M1-bar throttle, RSI filter, scaled burst lots, partial basket close."
...
{
   GBT_PROBE_TREND_ONLY       = 0,
   GBT_PROBE_TREND_AND_TICK   = 1
};
...
input group "=== RSI FILTER (v5) ==="
input bool   InpUseRSIFilter             = true;
input int    InpRSIPeriod                = 14;
input ENUM_TIMEFRAMES InpRSITF           = PERIOD_M5;
input double InpRSIOverbought            = 72.0;   // skip BUY above this
input double InpRSIOversold              = 28.0;   // skip SELL below this
input bool   InpUseRSITrendConfirm       = false;  // require RSI>50 for BUY / <50 for SELL
input double InpRSITrendMid              = 50.0;
...
input double InpMaxSpreadATRRatio        = 0.25;
...
input double InpBurstLotScale            = 1.0;    // v5: multiply each consecutive burst add (1.0 = flat)
input double InpMaxLot                   = 1.00;
input int    InpMaxPositionsPerSide      = 3;
...
input double InpProbeConfirmSpreadMult   = 0.80;
input double InpProbeConfirmATRMult      = 0.0;    // v5: use ATR*mult as confirm if > 0
...
input group "=== PER-BAR THROTTLE (v5) ==="
input bool   InpUsePerBarThrottle        = true;   // limit 1 probe/burst open per M1 bar per direction
...
input group "=== PARTIAL CLOSE (v5) ==="
input bool   InpUsePartialClose          = false;  // close a portion of basket at first target
input double InpPartialCloseAtMoney      = 0.70;   // close InpPartialClosePercent when basket profit >= this
input double InpPartialClosePercent      = 50.0;   // percent of positions to close (by count)
...
input double InpDailyLossLimitMoney      = 150.0;
...
int      g_burstAddCount[2];          // v5: tracks consecutive burst adds per direction
bool     g_partialDone[2];            // v5: partial close already executed this basket
datetime g_lastProbeBarTime[2];       // v5: per-bar throttle for probe
datetime g_lastBurstBarTime[2];       // v5: per-bar throttle for burst
...
bool RefreshMarketData()
{
   ...
   if(InpUseRSIFilter)
   {
      if(CopyBuffer(hRSI, 0, 0, 5, bufRSI) < 5)
      {
         g_lastStatus = "Waiting RSI";
         return false;
      }
   }
...
bool RSI_OK(int dir)
{
   ...
   if(dir > 0 && rsi >= InpRSIOverbought)
   {
      g_lastStatus = StringFormat("RSI overbought %.1f", rsi);
      return false;
   }

   if(dir < 0 && rsi <= InpRSIOversold)
   {
      g_lastStatus = StringFormat("RSI oversold %.1f", rsi);
      return false;
   }

   if(InpUseRSITrendConfirm)
   {
      if(dir > 0 && rsi < InpRSITrendMid)
      {
         g_lastStatus = StringFormat("RSI below trend mid %.1f", rsi);
         return false;
      }

      if(dir < 0 && rsi > InpRSITrendMid)
      {
         g_lastStatus = StringFormat("RSI above trend mid %.1f", rsi);
         return false;
      }
   }
...
bool EntryCandleOK(int dir)
{
   ...
   if(dir > 0)
   {
      ...
      if(InpRequireCloseBeyondPrev) ok = ok && c > h2;
      return ok;
   }
   ...
   {
      bool ok = c < o && c < bufEntryFast[1] && bufEntryFast[1] < bufEntrySlow[1];
      if(InpRequireCloseBeyondPrev) ok = ok && c < l2;
      return ok;
   }
...
int EntrySignal()
{
   ...
   if(InpSignalMode == GBT_BUY_ONLY  && dir < 0) return 0;
   if(InpSignalMode == GBT_SELL_ONLY && dir > 0) return 0;
   ...
   if(!ADXOK(dir))        return 0;
   if(!RSI_OK(dir))       return 0;   // v5
   if(!EntryCandleOK(dir)) return 0;
...
int CountPositionsByType(long typeFilter)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionGetInteger(POSITION_TYPE) == typeFilter)
         count++;
   }
...
{
   if(type == POSITION_TYPE_BUY)  return (tick.bid - openPrice) / _Point;
   if(type == POSITION_TYPE_SELL) return (openPrice - tick.ask) / _Point;
   return 0.0;
}
...
// v5: scaled burst lot based on how many adds have been made in this basket
double CalculateBurstLotScaled(int dir, double entry, double sl, int addIndex)
{
   double base = InpBurstLot;
   ...
   if(InpLotMode == GBT_LOT_RISK_PERCENT)
   {
      ...
      if(riskMoney > 0.0 && sl > 0.0)
      {
         ...
         if(OrderCalcProfit(orderType, _Symbol, 1.0, entry, sl, profitForOneLot))
         {
            ...
            if(lossPerLot > 0.0) base = riskMoney / lossPerLot;
         }
      }
   }

   // Apply pyramid scaling: lot = base * scale^addIndex
   double scale = (InpBurstLotScale > 0.0 ? InpBurstLotScale : 1.0);
   double scaled = base * MathPow(scale, (double)addIndex);

   return NormalizeVolumeFloor(scaled);
}
...
double CalculateLot(int dir, double entry, double sl, bool isProbe)
{
   if(isProbe)
   {
      ...
      return NormalizeVolumeFloor(lot);
   }

   // For burst, use current add count
   int idx = DirToIndex(dir);
   return CalculateBurstLotScaled(dir, entry, sl, g_burstAddCount[idx]);
}
...
bool OpenTrade(int dir, bool isProbe)
{
   ...
   if(InpUsePerBarThrottle)
   {
      ...
      if(isProbe)
      {
         if(currentBar == g_lastProbeBarTime[idx])
         {
            g_lastStatus = DirName(dir) + " probe throttled (same bar)";
            return false;
         }
      }
      else
      {
         if(currentBar == g_lastBurstBarTime[idx])
         {
            g_lastStatus = DirName(dir) + " burst throttled (same bar)";
            return false;
         }
      }
   }
   ...
   string comment = (isProbe ? "GBT5 PROBE " : "GBT5 BURST ");
   comment += DirName(dir);
   ...
   if(!sent || !TradeRetcodeOK())
   {
      ...
      if(InpDebug) Print(InpEAName, ": ", g_lastStatus);
      return false;
   }
   ...
   // v5: update per-bar throttle timestamps
   if(InpUsePerBarThrottle)
   {
      datetime currentBar = iTime(_Symbol, InpEntryTF, 0);
      if(isProbe)
         g_lastProbeBarTime[idx] = currentBar;
      else
         g_lastBurstBarTime[idx] = currentBar;
   }
   ...
   if(isProbe)
   {
      ...
      g_burstAddCount[idx]   = 0;    // v5: reset burst scaling counter
      g_partialDone[idx]     = false;// v5: reset partial close flag
   }
   else
   {
      g_burstAddCount[idx]++;        // v5: increment for next add scaling
   }
   ...
   if(InpDebug)
   {
      Print(InpEAName,
            ": opened ",
            (isProbe ? "PROBE " : StringFormat("BURST[%d] ", g_burstAddCount[idx])),
            DirName(dir),
   ...
   if(InpEnableAlerts) Alert(InpEAName + " " + comment);
...
bool ClosePositionByTicket(ulong ticket, string reason)
{
   ...
   {
      if(InpDebug) Print(InpEAName, ": closed ticket ", ticket, " | ", reason);
      return true;
   }

   if(InpDebug) Print(InpEAName, ": close failed ticket ", ticket, " | ", trade.ResultRetcodeDescription());
   return false;
}
...
void ClosePositionsByType(long typeFilter, string reason)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionGetString(POSITION_SYMBOL)  != _Symbol)       continue;
      if(PositionGetInteger(POSITION_MAGIC)  != InpMagicNumber) continue;
      if(PositionGetInteger(POSITION_TYPE)   != typeFilter)    continue;

      ClosePositionByTicket(ticket, reason);
   }
}

// v5: partial close - close approx InpPartialClosePercent of positions by count
void PartialCloseBasket(long typeFilter, string reason)
{
   // Collect tickets sorted oldest first
   ulong tickets[];
   datetime times[];
   int total = 0;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      ArrayResize(times,   total + 1);
      tickets[total] = ticket;
      times[total]   = (datetime)PositionGetInteger(POSITION_TIME);
      total++;
   }

   if(total == 0) return;

   // Simple insertion sort by open time (oldest first to close oldest positions)
   for(int a = 0; a < total - 1; a++)
      for(int b = a + 1; b < total; b++)
         if(times[b] < times[a])
         {
            ulong   tmpT = tickets[a]; tickets[a] = tickets[b]; tickets[b] = tmpT;
   ...
   for(int i = 0; i < closeCount; i++)
      ClosePositionByTicket(tickets[i], reason);
}
...
{
   double base = InpProbeConfirmPoints;

   // v5: optionally derive from current ATR
   if(InpProbeConfirmATRMult > 0.0)
   {
      double atrPts = ATRPoints();
      if(atrPts > 0.0)
         base = MathMax(base, atrPts * InpProbeConfirmATRMult);
   }

   return MathMax(base, (double)CurrentSpreadPoints() * InpProbeConfirmSpreadMult);
}
...
void RegisterBasketLoss(int dir, string reason)
{
   ...
   if(InpDebug)
   {
      Print(InpEAName,
            ": ", DirName(dir),
            " failed -> pause ", pauseSec, " sec | ", reason);
   }
}
...
void ResetDirectionIfFlat(int dir)
{
   ...
   if(CountPositionsByType(type) == 0)
   {
      ...
      g_basketPeakProfit[idx]= 0.0;
      g_lastAddMS[idx]       = 0;
      g_burstAddCount[idx]   = 0;  // v5
      g_partialDone[idx]     = false; // v5
   }
}
...
void ManageDirection(int dir, int freshSignal)
{
   ...
   // v5: partial close check (before full basket exits)
   if(InpUsePartialClose &&
      !g_partialDone[idx] &&
      profit >= InpPartialCloseAtMoney &&
      count > 1)
   {
      PartialCloseBasket(type, "partial close target");
      g_partialDone[idx] = true;

      if(InpDebug)
         Print(InpEAName, ": ", DirName(dir), " partial close executed profit=", DoubleToString(profit, 2));
   }
   ...
   if(!g_confirmed[idx])
   {
      ...
      if(probePts >= confirmPts || profit >= InpProbeConfirmMoney)
      {
         ...
         if(InpDebug)
         {
            Print(InpEAName,
                  ": ", DirName(dir),
                  " probe confirmed | pts=", DoubleToString(probePts, 1),
                  " money=", DoubleToString(profit, 2),
                  " confirmPts=", DoubleToString(confirmPts, 1));
         }
      }
   ...
   if(InpAddOnlyIfPriceImproves || InpMinSameSideSpacingPoints > 0.0)
   {
      ...
      if(SymbolInfoTick(_Symbol, tick))
      {
         ...
         {
            if(dir > 0 && priceNow <= latestEntryPrice) return;
            if(dir < 0 && priceNow >= latestEntryPrice) return;
         }
...
void DrawDashboard()
{
   ...
   int buyCount_  = CountPositionsByType(POSITION_TYPE_BUY);
   int sellCount_ = CountPositionsByType(POSITION_TYPE_SELL);

   int      bCount; double bLots; double bProfit; datetime bOld; ulong bTicket; double bLatest; datetime bLatestTime;
   int      sCount; double sLots; double sProfit; datetime sOld; ulong sTicket; double sLatest; datetime sLatestTime;
   ...
   double rsiVal = 0.0;
   if(InpUseRSIFilter && ArraySize(bufRSI) > 1) rsiVal = bufRSI[1];

   string text = StringFormat(
      "%s v5\nSymbol: %s | Spread: %d/%d | ATR: %.1f | RSI: %.1f | Status: %s\n"
      "B/S/T pos: %d/%d/%d | Lots %.2f/%.2f | B P/L %.2f | S P/L %.2f\n"
      "Confirmed B/S: %s/%s | BurstAdds B/S: %d/%d | Partial B/S: %s/%s\n"
      "Pause B/S: %d/%d sec | Global pause: %d sec\n"
      ...
      rsiVal,
      g_lastStatus,
      buyCount_, sellCount_, CountAllManagedPositions(),
      TotalManagedLots(), InpMaxTotalLots,
      bProfit, sProfit,
      g_confirmed[0] ? "YES" : "NO",
      g_confirmed[1] ? "YES" : "NO",
      g_burstAddCount[0], g_burstAddCount[1],
      g_partialDone[0] ? "Y" : "N",
      g_partialDone[1] ? "Y" : "N",
...
int OnInit()
{
   ...
   hRSI         = iRSI(_Symbol, InpRSITF, InpRSIPeriod, PRICE_CLOSE);  // v5
   ...
   g_confirmed[0]         = false;      g_confirmed[1]         = false;
   g_basketStartTime[0]   = 0;          g_basketStartTime[1]   = 0;
   g_basketPeakProfit[0]  = 0.0;        g_basketPeakProfit[1]  = 0.0;
   g_directionPauseUntil[0] = 0;        g_directionPauseUntil[1] = 0;
   g_probeFailsToday[0]   = 0;          g_probeFailsToday[1]   = 0;
   g_lastAddMS[0]         = 0;          g_lastAddMS[1]         = 0;
   g_burstAddCount[0]     = 0;          g_burstAddCount[1]     = 0;   // v5
   g_partialDone[0]       = false;      g_partialDone[1]       = false; // v5
   g_lastProbeBarTime[0]  = 0;          g_lastProbeBarTime[1]  = 0;   // v5
   g_lastBurstBarTime[0]  = 0;          g_lastBurstBarTime[1]  = 0;   // v5
   ...
   Print("============================================================");
   Print(InpEAName, " v5.00 initialized on ", _Symbol);
   Print("v5 features: per-bar throttle=", (InpUsePerBarThrottle ? "ON" : "OFF"),
         " | RSI filter=", (InpUseRSIFilter ? "ON" : "OFF"),
         " | BurstLotScale=", DoubleToString(InpBurstLotScale, 2),
         " | PartialClose=", (InpUsePartialClose ? "ON" : "OFF"));
   ...
   Print("Probe confirm: ",
         DoubleToString(InpProbeConfirmPoints, 1), " pts or $",
         DoubleToString(InpProbeConfirmMoney, 2),
         " | spread mult ", DoubleToString(InpProbeConfirmSpreadMult, 2),
         " | ATR mult ", DoubleToString(InpProbeConfirmATRMult, 2));
   Print("Basket: target $",
         DoubleToString(InpBasketProfitMoney, 2),
         " loss $", DoubleToString(InpBasketLossMoney, 2),
         " max life ", InpBasketMaxLifeSeconds, " sec");
   Print("Exposure: probe lot ",
         DoubleToString(InpProbeLot, 2),
         " burst lot ", DoubleToString(InpBurstLot, 2),
         " scale ", DoubleToString(InpBurstLotScale, 2),
         " max positions ", InpMaxTotalPositions,
         " max lots ", DoubleToString(InpMaxTotalLots, 2));
   Print("============================================================");
...
void OnDeinit(const int reason)
{
   ...
   if(hRSI         != INVALID_HANDLE) IndicatorRelease(hRSI);   // v5
}
                        ...
                        const MqlTradeResult &result)
{
      ...
      entry == DEAL_ENTRY_OUT_BY)
   {
      ...
      if(InpDebug)
      {
         long reason_ = HistoryDealGetInteger(deal, DEAL_REASON);
         Print(InpEAName,
               ": close deal ", deal,
               " profit=",   DoubleToString(profit, 2),
               " dayProfit=", DoubleToString(g_dayProfit, 2),
               " reason=", reason_);
      }
   }
}
