//+------------------------------------------------------------------+
//| MA Crossover EA - Multiple Pairs + Hedge + Auto-Close (v2.24)   |
//+------------------------------------------------------------------+
...
//=== CONFIG =========================================================
input string  EA_Name               = "MA_Crossover_EA_Hedge_Double_v2_24";
...
//+------------------------------------------------------------------+
// ManageTrades - new strategy:
//   * Zero active primaries  → open primary on signal (unchanged)
//   * < MaxPairs primaries   → open third trade only if signal matches
//                              existing primary direction AND a recovery
//                              is currently active.
//+------------------------------------------------------------------+
void ManageTrades(const ENUM_TRADE_SIGNAL sig)
{
   ...
   // === Open first primary (no positions yet) ===
   if(activePairs == 0)
   {
      double entryLots = LotSize;
      if(sig==SIGNAL_BUY)  { if(OpenTrade(ORDER_TYPE_BUY,  entryLots, false, 0)) SendSignal("BUY");  }
      else if(sig==SIGNAL_SELL) { if(OpenTrade(ORDER_TYPE_SELL, entryLots, false, 0)) SendSignal("SELL"); }
      return;
   }

   // === Consider a third (additional primary) trade ===
   if(activePairs < MaxPairs)
   {
      // Condition A: signal must match the direction of the existing primary
      int firstPrimaryType = FirstActivePrimaryType();
      bool signalMatchesPrimary =
         (firstPrimaryType == POSITION_TYPE_BUY  && sig == SIGNAL_BUY)  ||
         (firstPrimaryType == POSITION_TYPE_SELL && sig == SIGNAL_SELL);

      // Condition B: at least one recovery trade must be active
      bool recoveryActive = AnyRecoveryActive();

      if(signalMatchesPrimary && recoveryActive)
      {
         double entryLots = LotSize;
         if(sig==SIGNAL_BUY)  { if(OpenTrade(ORDER_TYPE_BUY,  entryLots, false, 0)) SendSignal("BUY (3rd trade)");  }
         else if(sig==SIGNAL_SELL) { if(OpenTrade(ORDER_TYPE_SELL, entryLots, false, 0)) SendSignal("SELL (3rd trade)"); }
      }
   }
}
...
void OnTick()
{
   ...
   for(int i=0; i<ArraySize(trackedTickets); i++)
   {
      ...
      double profit = PositionGetDouble(POSITION_PROFIT);
      if(profit <= 0.0) continue; // not yet positive

      // Primary is positive - close its linked recovery immediately
      for(int j=ArraySize(trackedTickets)-1; j>=0; j--)
      {
         ...
         if(PositionSelectByTicket(recTicket))
         {
            if(trade.PositionClose(recTicket))
            {
               PrintFormat("[%s] Auto-closed recovery %I64u - primary %I64u returned to profit %.2f",
                           EA_Name, recTicket, primaryTicket, profit);
               UntrackPosition(recTicket);
            }
         }
      }
   ...
   for(int i=0; i<ArraySize(trackedTickets); i++)
   {
      ...
      trackedOpenCandle[i] = LONG_MAX; // mark processed
   }
