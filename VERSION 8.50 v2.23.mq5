//+------------------------------------------------------------------+
//| MA Crossover EA - Multiple Pairs + Hedge + Auto-Close (v2.23)   |
//+------------------------------------------------------------------+
...
//=== CONFIG =========================================================
input string  EA_Name               = "MA_Crossover_EA_Hedge_Double_v2_23";
...
input double  HedgeLotMultiplier    = 2.0;  // Kept for reference; recovery now uses LotSize
input int     MaxPairs              = 2;    // Max simultaneous trade pairs (primary + hedge each)
...
bool     trackedIsSecond[];   // true if this is a hedge/recovery trade (explicitly set at open)
...
// Tracking helpers
//+------------------------------------------------------------------+
// isSecond must be passed explicitly - volume comparison is unreliable when
// hedge lots equal primary lots (all hedges now use LotSize, not a multiplier).
void TrackPositionOpen(ulong ticket, bool isSecond)
{
   if(ticket==0) return;
...
//+------------------------------------------------------------------+
// isSecond = true  when opening a hedge/recovery against an existing position
// isSecond = false when opening a fresh primary trade on a signal
bool OpenTrade(const ENUM_ORDER_TYPE type, double lots, bool isSecond = false)
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      {
         TrackPositionOpen(t, isSecond);
         break;
      }
   }

   PrintFormat("[%s] %s opened (%.2f) SL/TP set isSecond=%s", EA_Name, EnumToString(type), lots, isSecond ? "Y":"N");
   return true;
}
...
void ManageTrades(const ENUM_TRADE_SIGNAL sig)
{
   ...
   // Open new primary trade if active pairs < MaxPairs and a valid signal exists
   if(CountActivePairs() < MaxPairs)
   {
      double entryLots = LotSize;
      if(sig==SIGNAL_BUY) { if(OpenTrade(ORDER_TYPE_BUY, entryLots, false)) SendSignal("BUY"); }
      else if(sig==SIGNAL_SELL) { if(OpenTrade(ORDER_TYPE_SELL, entryLots, false)) SendSignal("SELL"); }
      return;
   }

   // If 1 position exists -> check if it is negative; if negative open opposite same-size lot and close the original
   if(CountOpenPositions() == 1)
   {
      ...
      for(int i=PositionsTotal()-1;i>=0;i--)
      {
         ...
         if(profit < 0.0)
         {
            // Recovery opens at the original LotSize - never cascade-double
            double newLots = LotSize;

            // Broker max lot safety check
            double vol_max = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
            if(newLots > vol_max)
            {
               PrintFormat("[%s] Lot size %.2f exceeds broker max %.2f - skipping hedge", EA_Name, newLots, vol_max);
               break;
            }

            ENUM_ORDER_TYPE openType = (ptype == POSITION_TYPE_BUY) ? ORDER_TYPE_SELL : ORDER_TYPE_BUY;

            if(DebugMode) PrintFormat("[%s] Existing pos (ticket=%I64u) is negative (%.2f). Opening opposite hedge lot %.2f", EA_Name, t, profit, newLots);

            if(OpenTrade(openType, newLots, true))
            {
               ...
               if(PositionSelectByTicket(t))
               {
                  ...
                  {
                     PrintFormat("[%s] Closed original (ticket=%I64u) after opening hedge", EA_Name, t);
                     UntrackPosition(t);
                  }
                  else
                  {
                     PrintFormat("[%s] Failed to close original (ticket=%I64u) after opening hedge", EA_Name, t);
                  }
               }
            }
         }
   ...
   // If MaxPairs reached, do not open more primaries
}
...
void OnTick()
{
   ...
   // =========================
   // AUTO-CLOSE HEDGE WHEN PRIMARY RETURNS TO POSITIVE (NEW v2.22)
   // =========================
   // For every primary (non-hedge) position that is currently in profit,
   // immediately close any open hedge/recovery in the opposite direction.
   // This locks in the "both-at-SL protection" strategy and caps max loss.
   for(int i=0; i<ArraySize(trackedTickets); i++)
   {
      if(trackedIsSecond[i]) continue; // look at primary positions only

      ulong firstTicket = trackedTickets[i];
      if(!PositionSelectByTicket(firstTicket)) continue;

      double profit = PositionGetDouble(POSITION_PROFIT);
      if(profit <= 0.0) continue; // primary not yet positive

      // Primary is positive - close any opposite hedge/recovery
      for(int j=ArraySize(trackedTickets)-1; j>=0; j--)
      {
         if(!trackedIsSecond[j]) continue; // only hedge/recovery positions
         if(trackedType[j] == trackedType[i]) continue; // must be opposite direction
         ...
         if(PositionSelectByTicket(secondTicket))
         {
            ...
            {
               PrintFormat("[%s] Auto-closed hedge/recovery (ticket=%I64u) - primary ticket=%I64u returned to profit %.2f",
                           EA_Name, secondTicket, firstTicket, profit);
   ...
   // Count open primaries and open hedges from the tracking arrays.
   // Open a hedge for any primary that is currently negative and has no hedge yet.
   {
      int numPrimaries = 0, numHedges = 0;
      for(int i = 0; i < ArraySize(trackedTickets); i++)
         if(PositionSelectByTicket(trackedTickets[i]))
            (trackedIsSecond[i] ? numHedges : numPrimaries)++;

      if(numPrimaries > numHedges)
      {
         for(int p=PositionsTotal()-1; p>=0; p--)
         {
            ...
            // Skip positions that are themselves tracked as hedges
            bool isHedge = false;
            for(int i = 0; i < ArraySize(trackedTickets); i++)
               if(trackedTickets[i]==orig_ticket && trackedIsSecond[i]) { isHedge=true; break; }
            if(isHedge) continue;
            ...
            // Recovery uses original LotSize - no cascading doubling
            double newLots = LotSize;

            // Broker max lot safety check - prevents broker rejection
            double vol_max = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
            if(newLots > vol_max)
            {
               PrintFormat("[%s] Lot size %.2f exceeds broker max %.2f - skipping hedge", EA_Name, newLots, vol_max);
            ...
            if(OpenTrade(openType, newLots, true))
            {
               PrintFormat("[%s] Hedge opened against ticket %I64u (lots=%.2f)", EA_Name, orig_ticket, newLots);
            }

            break; // one hedge per tick
         }
      }
   }
