//+------------------------------------------------------------------+
//| GoldPulse_BurstTrend_v5_0.mq5                                    |
//| Revamped by Grok - April 2026                                    |
//| v5: Asymmetric L/S + Momentum Confirm + Partial Close + Vol Gate |
//| This should finally give positive expectancy                     |
//+------------------------------------------------------------------+
#property copyright "Wayne Heyliger / Grok Revamp v5"
#property version   "5.00"
#property strict
#property description "v5: Asymmetric rules, momentum candle confirm, volatility gate, partial close"
...
enum ENUM_GBT_ENTRY_MODEL
{
   GBT_ENTRY_MODEL_TICK    = 0, // original tick-driven entry
   GBT_ENTRY_MODEL_NEW_BAR = 1, // evaluate entry on closed bar only (default)
   GBT_ENTRY_MODEL_HYBRID  = 2  // new-bar primary; optional tick fallback
};
//====================================================================
// INPUTS - TUNED FOR BETTER PERFORMANCE (key changes here)
//====================================================================
input group "=== EA IDENTITY ==="
input string InpEAName                  = "GoldPulse_BurstTrend_v5_0";
...
input int    InpTrendSlopeLookback       = 4;
input bool   InpUseBiasFilter            = true;
input int    InpBiasEMA                  = 200;       // ← ADDED (was missing)

input group "=== ADX / DI FILTER ==="
input bool   InpUseADXFilter             = true;
input int    InpADXPeriod                = 18;
input double InpADXMin                   = 17.0;
...
input group "=== TICK IMPULSE ==="
input ENUM_GBT_PROBE_STYLE InpProbeStyle = GBT_PROBE_TREND_ONLY;
input int    InpTickLookback             = 6;
input double InpTickImpulsePoints        = 22.0;
input int    InpMinDirectionalTicks      = 3;
input double InpMaxImpulseSeconds        = 3.0;

input group "=== ENTRY MODEL ==="
input ENUM_GBT_ENTRY_MODEL InpEntryModel = GBT_ENTRY_MODEL_NEW_BAR;
input bool   InpUseTickImpulseInHybrid   = false;

input group "=== NEW v5 FILTERS ==="
input bool   InpRequireCandleConfirm     = true;      // Momentum confirmation
input int    InpMinMomentumBars          = 1;
input bool   InpUseVolFilter             = true;
input double InpMinATRForTrade           = 75.0;

input group "=== ASYMMETRIC BASKET (v5) ==="
input bool   InpUseAsymmetricRules       = true;
input double InpLongBasketProfitMoney    = 18.0;
input double InpShortBasketProfitMoney   = 11.5;

input group "=== PARTIAL CLOSE (v5) ==="
input bool   InpUsePartialClose          = true;
input double InpPartialCloseProfitMoney  = 9.0;
...
input double InpMinATRPoints             = 60.0;       // loosened
...
input int    InpMaxPositionsPerSide      = 4;          // increased
input int    InpMaxTotalPositions        = 4;
input double InpMaxTotalLots             = 0.05;       // increased

input group "=== PROBE -> BURST ==="
input double InpProbeConfirmPoints       = 35.0;       // improved
input double InpProbeConfirmMoney        = 0.80;
input double InpProbeConfirmSpreadMult   = 0.80;
input double InpProbeFailPoints          = 180.0;
input double InpProbeFailMoney           = -2.50;
input int    InpProbeMaxSeconds          = 120;
input double InpProbeTimeoutMinPoints    = -15.0;
input int    InpDirectionPauseAfterProbeFailSec = 90;
...
input int    InpMinSecondsBetweenAdds    = 8;
input double InpMinBasketProfitToAdd     = 0.0;
input double InpMinSameSideSpacingPoints = 25.0;
input bool   InpRequireFreshSignalToAdd  = true;
input bool   InpAddOnlyIfPriceImproves   = true;
// ← removed duplicate InpAddOnlyOnNewBar here
...
input double InpATRSLMultiplier          = 1.10;       // more breathing room
input bool   InpUseBreakEven             = true;
input double InpBETriggerPoints          = 70.0;
input double InpBEPlusPoints             = 10.0;
input bool   InpUseTrailing              = true;
input double InpTrailStartPoints         = 110.0;
input double InpTrailATRMultiplier       = 0.90;
input double InpTrailMinPoints           = 80.0;
...
input double InpBasketProfitMoney        = 6.50;       // fallback when InpUseAsymmetricRules=false
input double InpBasketLossMoney          = -4.50;
input bool   InpUseBasketProfitTrail     = true;
input double InpBasketTrailStartMoney    = 6.50;
input double InpBasketTrailGivebackMoney = 3.00;
input int    InpBasketMaxLifeSeconds     = 900;
input int    InpCooldownAfterBasketProfitSec = 15;
input int    InpCooldownAfterBasketLossSec   = 120;

input group "=== REALTICK HARD MODE ==="
input bool InpRealTickHardMode      = true;
input bool InpOneProbePerBar        = true;
input bool InpAddOnlyOnNewBar       = true;   // kept here only
input bool InpForceTrendOnlyInHard  = true;
...
input double InpDailyLossLimitMoney      = 120.0;
input double InpDailyLossLimitPercent    = 1.80;
...
input int InpMinHoldSecondsBeforeFail = 45;

//====================================================================
// GLOBALS / INDICATORS (unchanged except minor)
//====================================================================
#define GBT_TICK_BUF 128

int hTrendFast, hTrendSlow, hTrendAnchor, hBiasEMA, hEntryFast, hEntrySlow, hATR, hADX;

double bufTrendFast[], bufTrendSlow[], bufTrendAnchor[], bufBiasEMA[];
double bufEntryFast[], bufEntrySlow[], bufATR[], bufADX[], bufPlusDI[], bufMinusDI[];

MqlRates ratesTrend[], ratesBias[], ratesEntry[];
...
long     g_lastAddMS[2];
long     g_lastEntryMS = 0;          // ← ADDED
long     g_lastTickMS  = 0;          // ← ADDED (used in many places)
...
datetime g_lastProbeBarBuy  = 0;
datetime g_lastProbeBarSell = 0;

bool     g_partialCloseDone[2];
...
//====================================================================
// SIGNAL LOGIC (restored + loosened filters)
//====================================================================
int TrendDirection()
{
...
int EntrySignalCore(bool enforceTickImpulse)
{
   ...
   // === v5 VOLATILITY GATE ===
   if(InpUseVolFilter && ATRPoints() < InpMinATRForTrade)
   {
      g_lastStatus = "ATR too low for trade";
      return 0;
   }

   bool useTickImpulse = enforceTickImpulse && (InpProbeStyle == GBT_PROBE_TREND_AND_TICK);
   ...
   // === v5 MOMENTUM CANDLE CONFIRM ===
   if(InpRequireCandleConfirm && !EntryCandleOK(dir))
   {
      g_lastStatus = "Waiting momentum candle";
      return 0;
   }
...
void ManageDirection(int dir, int freshSignal, bool newBar)
{
   int idx = DirToIndex(dir);
   long type = DirToPositionType(dir);
   ...
   bool hasBasket = BasketStats(type, count, lots, profit, oldestTime, oldestTicket, latestEntryPrice, latestTime);
   ...
   {
      // === v5 ASYMMETRIC BASKET TARGET ===
      double target = InpUseAsymmetricRules ?
                     (dir > 0 ? InpLongBasketProfitMoney : InpShortBasketProfitMoney)
   ...
   // === v5 PARTIAL CLOSE ===
   if(InpUsePartialClose && !g_partialCloseDone[idx] &&
      profit >= InpPartialCloseProfitMoney && count > 1)
   {
      int halfCount = count / 2;
      int closed = 0;
      for(int pc = PositionsTotal() - 1; pc >= 0 && closed < halfCount; pc--)
      {
         ulong pcTicket = PositionGetTicket(pc);
         if(pcTicket == 0) continue;
         if(!PositionSelectByTicket(pcTicket)) continue;
         if(PositionGetString(POSITION_SYMBOL) != _Symbol) continue;
         if(PositionGetInteger(POSITION_MAGIC) != InpMagicNumber) continue;
         if(PositionGetInteger(POSITION_TYPE) != type) continue;
         if(ClosePositionByTicket(pcTicket, "partial close"))
            closed++;
      }
      g_partialCloseDone[idx] = true;
      if(InpDebug && closed > 0)
         Print(InpEAName, ": Partial close triggered - closed ", closed, " of ", count, " positions");
   }
   ...
   if(!g_confirmed[idx])
   {
      ...
      if(probePts >= confirmPts || profit >= InpProbeConfirmMoney)
      {
         ...
         if(InpDebug)
            Print(InpEAName, ": ", DirName(dir), " PROBE CONFIRMED");
      }
      else if(ageSec >= InpMinHoldSecondsBeforeFail &&
             (probePts <= -MathAbs(InpProbeFailPoints) || profit <= InpProbeFailMoney))
      {
      ...
      else if(ageSec >= MathMax(InpProbeMaxSeconds, InpMinHoldSecondsBeforeFail) &&
              probePts < InpProbeTimeoutMinPoints)
      {
   ...
   if(!g_confirmed[idx]) return;

   // === BURST ADD PHASE ===
   if(count >= InpMaxPositionsPerSide || CountAllManagedPositions() >= InpMaxTotalPositions) return;
   if(TotalManagedLots() + InpBurstLot > InpMaxTotalLots + 0.0000001) return;
   if(profit < InpMinBasketProfitToAdd) return;
   if(InpRequireFreshSignalToAdd && freshSignal != dir) return;
   if((g_lastTickMS - g_lastAddMS[idx]) < (long)InpMinSecondsBetweenAdds * 1000) return;
   if(InpAddOnlyOnNewBar && !newBar) return;
   ...
   if(InpAddOnlyIfPriceImproves || InpMinSameSideSpacingPoints > 0.0)
   {
      ...
      if(SymbolInfoTick(_Symbol, tick))
      {
         ...
         {
            if((dir > 0 && priceNow <= latestEntryPrice) || (dir < 0 && priceNow >= latestEntryPrice)) return;
         }
         if(InpMinSameSideSpacingPoints > 0.0 && MathAbs(priceNow - latestEntryPrice) < InpMinSameSideSpacingPoints * _Point)
            return;
      }
   }

   for(int n = 0; n < MathMax(1, InpBurstOrdersPerAdd); n++)
   {
      if(!OpenTrade(dir, false)) break;
      g_lastAddMS[idx] = g_lastTickMS;
   }
}

bool TryOpenProbe(int signal, bool newBar)
{
   ...
   datetime barTime = (ArraySize(ratesEntry) > 1 ? ratesEntry[1].time : 0);

   if(InpRealTickHardMode && InpOneProbePerBar && barTime > 0)
   {
      if(signal > 0 && g_lastProbeBarBuy == barTime)  return false;
      if(signal < 0 && g_lastProbeBarSell == barTime) return false;
   }

   bool ok = OpenTrade(signal, true);

   if(ok && barTime > 0 && InpRealTickHardMode && InpOneProbePerBar)
   {
      if(signal > 0) g_lastProbeBarBuy = barTime;
      else           g_lastProbeBarSell = barTime;
   }
...
int OnInit()
{
   ...
   g_lastAddMS[0] = 0;
   g_lastAddMS[1] = 0;
   g_partialCloseDone[0] = false;
   g_partialCloseDone[1] = false;
...
bool IsNewClosedEntryBar()
{
   if(ArraySize(ratesEntry) < 2) return false;
   datetime barTime = ratesEntry[1].time;
   if(barTime <= 0) return false;

   if(barTime != g_lastSignalBarTime)
   {
...
void OnTick()
{
   ...
   bool newBar = IsNewClosedEntryBar();
   int signal = EntrySignalCore(true);

   TryOpenProbe(signal, newBar);           // probe first
   ManageDirection(1, signal, newBar);
   ManageDirection(-1, signal, newBar);
